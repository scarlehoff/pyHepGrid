...
"""A grid submission script using ganga, modified for MCFM"""
...
if not warmup: # attempt to find the grid files automatically
    warmuptar = 'output'+sys.argv[1]+'-w'+'.tar.gz'
...
os.system('chmod +x mcfm_omp')

# COMMAND GOES HERE
command = ''

command += './mcfm_omp '+sys.argv[1]
