...
class NNLOJET(ProgramInterface):
    ...
    def check_for_existing_output_local(self, r, rname, baseSeed, producRun):
        ...
        runcard = PROGRAMruncard(
            runcard_file=os.path.join(local_dir_name, r),
            logger=logger,
            grid_run=False,
        ...
        existing_seeds = set([int(logseed_regex.search(i).group(1)) for i in logs])
    ...
    def _bring_warmup_files(self, runcard, rname, check_only=False, multichannel=False):
        """
        ...
        if not success and not check_only:
            if (
                self._press_yes_to_continue(
                    "Grid files failed to copy. " "Try backups from individual sockets?"
                )
                == 0
            ):
                ...
                if len(backups) == 0:
                    logger.critical(
                        "No backups found. " "Did the warmup complete successfully?"
                    )
                else:
                    backup_files = backups.split()
                    for idx, backup in enumerate(backup_files):
                        logger.info(
                            "Attempting backup " "{1} [{0}]".format(idx + 1, backup)
                        )
        ...
        if not success:
            logger.critical(
                "Grid files failed to copy. " "Did the warmup complete successfully?"
            )
        else:
        ...
        if gridFiles == [] and not check_only:  # No grid files found
            logger.critical(
                "Grid files not found in warmup tarfile. "
    ...
    def get_grid_from_stdout(self, jobid, jobinfo):
        ...
        try:
            ...
            logger.critical(
                "No grid filename found in stdout logs. " "Did the warmup write a grid?"
            )

        result = re.search(
            "vegas warmup to stdout(.*)End", stdout, flags=re.S
    ...
    def init_warmup(self, provided_warmup=None, continue_warmup=False, local=False):
        ...
        if local:
            self.init_local_warmups(
                provided_warmup=provided_warmup, continue_warmup=continue_warmup
            )
            return
        ...
        for idx, i in enumerate(rncards):
            ...
            if self.overwrite_warmup:
                ...
                if self.gridw.checkForThis(checkname, header.grid_warmup_dir):
                    logger.info(
                        "Warmup found in GFAL:{0}!".format(header.grid_warmup_dir)
                    )
            ...
            logger.info(
                "Sending {0} to gfal {1}/".format(tarfile, header.grid_input_dir)
            )
    ...
    def init_single_local_production(self, runcard, tag, provided_warmup=False):
        ...
        if provided_warmup:
            ...
            if runcard_obj.warmup_filename() not in rundirfiles:
                logger.critical(
                    "No warmup found in run folder " "and no warmup provided manually"
                )
    ...
    def init_production(self, provided_warmup=None, continue_warmup=False, local=False):
        ...
        for idx, i in enumerate(rncards):
            ...
            if provided_warmup:
                ...
                match, local = self._get_local_warmup_name(
                    runcard_obj.warmup_filename(), header.provided_warmup_dir
                )
                warmupFiles = [match]
            else:
    ...
    def _get_local_warmup_name(self, matchname, provided_warmup):
        ...
        if os.path.isdir(provided_warmup):
            ...
            for potfile in potential_files:
                if potfile.lower().startswith(matchname) and not any(
                    potfile.endswith(p) for p in exclude_patterns
                ):
            ...
            if len(matches) > 1:
                ...
                potmatches = sorted(
                    potential_files,
                    key=lambda x: SequenceMatcher(None, x, matchname_case).ratio(),
                    reverse=True,
                )
                print(potmatches)
                ...
                for pot in [
                    ...
                    for p in potmatches
                    if (
                        (SequenceMatcher(None, p, matchname_case).ratio() > 0.5)
                        and not (any([p.endswith(ext) for ext in exclude_patterns]))
                    )
                ]:
                    yn = input(
                        "Partial warmup match found: "
                ...
                if not match:
                    logger.info(
                        "No warmup matches found in directory {0}. "
                        "Searching subdirectories:".format(provided_warmup)
                    )
                    potmatches = []
                    for path, _subdirs, files in os.walk(provided_warmup):
                        for name in files:
                            if (
                                SequenceMatcher(None, name, matchname_case).ratio()
                                > 0.5
                            ):
                    ...
                    if matchname_case.split(".")[-1] in warmup_extensions:
                        potmatches = [
                            ...
                            if p[1]
                            .lower()
                            .endswith("." + matchname_case.split(".")[-1].lower())
                        ]

                    potmatches = sorted(
                        potmatches,
                        key=lambda x: SequenceMatcher(
                            None, x[1], matchname_case
                        ).ratio(),
                    )
                    ...
                    for pot in [
                        ...
                        for p in potmatches
                        if (
                            (SequenceMatcher(None, p, matchname).ratio() > 0.5)
                            and not (any([p.endswith(ext) for ext in exclude_patterns]))
                        )
                    ]:
                        yn = input(
        ...
        if SequenceMatcher(None, os.path.basename(match), matchname_case).ratio() <= 1:
            if "." + matchname_case.split(".")[-1] in warmup_extensions:
                ...
                suff = input(
                    ...
                    "or 'n' to provide your own filename.".format(matchname_case)
                )
                if suff in warmup_extensions:
                    ...
                    fn = input(
                        "Please enter the filename NNLOJET will expect "
                        "the warmup to have:"
                    )
                    copy(match, fn)
                    match = fn
            ...
            print(
                SequenceMatcher(None, os.path.basename(match), matchname_case).ratio()
            )
            print(sys.path[0])
