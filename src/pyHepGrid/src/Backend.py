...
class Backend(_mode):
    ...
    def _get_computing_element(self, id_example):
        """
        Given a list jobids, returns the computing elements they are being run
    ...
    def get_data(self, db_id, jobid=None, custom_get=None):
        """
        External interface for the get_data routines. If a custom_get is defined
        in the pyHepGrid.src.header, it will be used instead of the 'native'
        _get_data_{production/warmup}. Custom scripts need to have a public
        "do_finalise()" function for this to work
        """
        if custom_get:
            import importlib
            ...
            # continuing and call the corresponding get_function
