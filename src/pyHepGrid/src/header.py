...
head = importlib.import_module(
    header_mappings.get(grid_username, header_mappings["default"])
)
...
gsiftp = (
    "gsiftp://se01.dur.scotgrid.ac.uk/dpm/dur.scotgrid.ac.uk/home/" + "pheno/generated/"
)
...
# are
socket_exe = "{0}/socket_server.py".format(os.path.dirname(os.path.realpath(__file__)))
...
template_attributes = [
    i for i in template_namespace if not callable(getattr(template, i))
]
# remove modules from template namespace
template_attributes = [
    i for i in template_namespace if not isinstance(getattr(template, i), ModuleType)
]
...
for i in dir(head):
    if not i.startswith("__"):
        ...
        # the template.
        if (
            i not in template_attributes
            and not isinstance(attr, ModuleType)
            and not callable(attr)
            and i not in ["arcprodtable", "slurmprodtable"]
        ):
            logger.warning(
                "attribute {0} not present in {1}".format(i, template.__name__)
            )
            logger.info(
                "Please add it in before committing so you don't break "
                "compatibility(!)"
            )
...
for i in template_attributes:
    try:
        ...
        logger.info(
            f"Check that {head.__name__}.py file is up to date as "
...
if runcard_file:
    ...
    for attr_name in dir(runcard):
        if not attr_name.startswith("__") and not isinstance(
            getattr(runcard, attr_name), ModuleType
        ):
            if not hasattr(this_file, attr_name) and attr_name != "dictCard":
                logger.warning(
                    "{0} defined in {1}.py but not {2}.py.".format(
    ...
    for attr_name in additional_arguments:
        ...
        if not new:  # Not a new argument only defined at cmd line args
            try:
                ...
                logger.error(
                    f"Additional argument {attr_name} with value "
...
_slurmfilename = os.path.join(
    os.path.dirname(os.path.realpath(__file__)), slurm_template
)
with open(_slurmfilename) as template_file:
    SLURMSCRIPTDEFAULT = template_file.read()

_slurmfilename_production = os.path.join(
    os.path.dirname(os.path.realpath(__file__)), slurm_template_production
)
