...
def createdirs(currentdir, runcard, rtag):
    ...
    mkdir(copylogdir)
    logcheck = set(
        [logseed_regex.search(i).group(1) for i in get_PROGRAM_logfiles(logdir)]
    )
...
def pullrun(name, seed, run, tmpdir, subfolder, attempts=0):
        ...
        with tarfile.open(name, "r|gz") as tfile:
            for t in tfile:
                if t.name.endswith(".dat"):
                    ...
                    t.name.endswith(".log")
                    and "node_info" not in t.name
                    and "copies." not in t.name
                ):
                    tfile.extract(t, "../log/")
                    corrupted = False
    ...
    if corrupted:
        ...
        if DELETE_CORRUPTED:
            ...
            if verbose:
                print(
                    f"\033[91mDeleting {run}, seed {seed}. " f"Corrupted output\033[0m"
                )
...
def pull_folder(foldername, folders=None, pool=None, rtag=None):
    ...
    cmd_output = (
        subprocess.Popen(cmd, stdout=subprocess.PIPE).communicate()[0].decode("utf-8")
    )
    output = []
    ...
    if RECURSIVE:
        for output_folder in output_folders:
            if any([tag == output_folder for tag in folders]):
                pull_folder(
                    os.path.join(foldername, output_folder),
                    pool=pool,
                    rtag=output_folder,
                )
