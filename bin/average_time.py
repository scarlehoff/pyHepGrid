...
def add_runcard_directories(args):
    ...
    for runcard in runcard_import.dictCard:
        direc = finalise.get_output_dir_name(runcard) + "-{0}/log".format(
            runcard_import.dictCard[runcard]
        )
...
def print_full_output(directory, times, skip_count, total_runs, histogram):
    ...
    if histogram:
        ...
        pyHepGrid.src.gnuplot.do_plot(
            x, y, xlabel="No. hours", title="Run time histogram"
        )


def print_short_header():
    header = "{1:55} {0:17}      {2:17}".format("Average time", "Runcard", "Total time")
    print(header)
    print("=" * len(header))
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "directories", metavar="dir", help="Directories to search.", nargs="*"
    )
    parser.add_argument("--runcard", "-rc", help="runcard.py file to use.")
    parser.add_argument(
        "--verbose", "-v", help="Outputs a much longer summary.", action="store_true"
    )
    parser.add_argument(
        "--search",
        "-f",
