...
sys.path.append(os.path.dirname(os.path.dirname(os.path.abspath(__file__))))  # Yuck
...
def create_new_session_with_server(name, serverloc, tot, host, port):
    servercommand = "{0} -N {1} -H {2} -p {3}".format(serverloc, tot, host, port)
    os.system(
...
def create_NNLOJET_instance(port, host, no, tot, window_name, runcard, desktop, runloc):
    ...
    runcmd = (
        "'ssh {0} cd {1} && nice -n {5} ./NNLOJET -run {2} {3} "
        + "&& tmux kill-session -t {4}'"
    )
    runcmd = runcmd.format(
        desktop, runloc, runcard, socket_info, window_name, nice_priority
    )
    os.system(
        "tmux split-window -c {0} -t {2}: {1}".format(runloc, runcmd, window_name)
    )
    arrange_tmux_window(window_name)


def arrange_tmux_window(window_name):
    os.system("tmux select-layout -t {0}:0 tiled >/dev/null 2>&1".format(window_name))
...
def do_single_run(host, port, runcard, serverloc, tot, desktop_list, run_directory):
    ...
    create_new_session_with_server(window_name, serverloc, tot, host, valid_port)
    ...
    for no, desktop in enumerate(desktop_list):
        create_NNLOJET_instance(
            valid_port, host, no, tot, window_name, runcard, desktop, run_directory
        )
        print(
            "NNLOJET instance [{0}/{1}] submitted to {2}.".format(no + 1, tot, desktop)
        )
    arrange_tmux_window(window_name)
...
if __name__ == "__main__":
    for runcard in header.dictCard:
        ...
        do_single_run(host, port, runcard, serverloc, tot, desktop_list, run_directory)
