...
def setup_sockets(args, nnlojet_command, bring_status):
    ...
    if bring_status != 0:
        print_flush(
            "Not able to bring data from gfal, " "removing myself from the pool"
        )
...
def parse_arguments():
    ...
    parser.add_option(
        "-e", "--executable", help="Executable to be run", default="NNLOJET"
    )
...
def store_output(args, socketed=False, socket_config=""):
    ...
    if socketed:
        ...
        if socket_no == 1:
            ...
            subfolder = os.path.splitext(os.path.basename(output_file))[0].replace(
                ".tar", ""
            )
